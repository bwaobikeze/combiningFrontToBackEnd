...
const createQuote = async function (values) {
    try {
        ...
        console.log(values)
        let quote = await new QuoteModel({ userId:values.userId, gallonreq:values.gallonreq, deliveryaddress:values.deliveryaddress, deliverydate:values.deliverydate, suggestedpricepergallon:values.suggestedpricepergallon, totalamountdue:values.totalamountdue })
            ?.save()
